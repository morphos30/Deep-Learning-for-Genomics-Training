...
LOAD CONV ACTIVATION VALUES SAVED FROM GOOGLE COLAB
```{r}
np <- reticulate::import(module="numpy") # say 'no'
# data reading
conv1d_output <- np$load("/home/mourad/Downloads/conv_activ_pos.npy")

kernelSize=peakSize-dim(conv1d_output)[2]+1
print(kernelSize)

conv1d_output[1,1:10,1:5]
```
...
Plot first motif
```{r, plot_1stmotif}
motif1 <- new("pcm", mat=as.matrix(kernelPFMList[[4]]), name=ID(kernelPFMList[[1]]))
plot(motif1)
```
